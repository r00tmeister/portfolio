<template>
    <div
        class="divide-y divide-gray-200 overflow-hidden rounded-lg bg-black shadow sm:grid sm:grid-cols-2 sm:gap-px sm:divide-y-0  bg-transparent bg-opacity-20 backdrop:filter backdrop-blur-md ">
        <div v-for="(action, actionIdx) in actions" :key="action.title"
            :class="[actionIdx === 0 ? 'rounded-tl-lg rounded-tr-lg sm:rounded-tr-none' : '', actionIdx === 1 ? 'sm:rounded-tr-lg' : '', actionIdx === actions.length - 2 ? 'sm:rounded-bl-lg' : '', actionIdx === actions.length - 1 ? 'rounded-bl-lg rounded-br-lg sm:rounded-bl-none' : '', 'group relative bg-black bg-transparent bg-opacity-20 backdrop:filter backdrop-blur-md shadow-lg p-6 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-500']">
            <div>
                <span
            ...
            <div class="mt-8">
                ...
                <p class="mt-2 text-sm text-gray-500">Doloribus dolores nostrum quia qui natus officia quod et dolorem.
